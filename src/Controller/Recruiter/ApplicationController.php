...
{
    #[Route('/', name: 'app_recruiter_applications')]
    public function index(EntityManagerInterface $em, Request $request): Response
    {
        ...
        // Determine context
        if ($recruiter) {
            $filters['recruiter'] = $recruiter->getId();
            // If recruiter view, we shouldn't filter by employer unless we want intersection?
            // For now, assume Recruiter View excludes Employer filter unless explicit?
            // Check if Recruiter has Employer?
            // Let's stick to "Recruiter View" means seeing applications assigned to Recruiter.
            unset($filters['employer']); // Priorities Recruiter filter if both strictly not required
        } elseif ($employer) {
            $filters['employer'] = $employer->getId();
        } else {
            // Fallback or empty result
            // $filters['employer'] = null; // This might cause repository to return everything? 
            // Repository checks !empty($filters['employer']) so it won't filter.
            // We should probably force empty result if neither.
        ...
        // Status Counts
        if ($recruiter) {
            $rawStatusCounts = $em->getRepository(Application::class)->getRecruiterApplicationStatusCounts($recruiter->getId());
    ...
    #[Route('/{id}/review-details', name: 'app_recruiter_application_review_details', methods: ['GET'])]
    public function getReviewDetails(
        ...
        if ($employer) {
            ...
            // Logic for Recruiter review? Maybe they review as themselves?
            // Current Review entity might only have 'employer' field.
            // If Review doesn't support Recruiter, we might skip or use 'reviewedBy' context.
            // For now, let's assume if it's Recruiter, they don't have this feature or it needs entity update.
        ...
        $existingReview = $em->getRepository(Review::class)->findOneBy($criteria);
    ...
    #[Route('/{id}/document-requests', name: 'app_recruiter_application_document_requests', methods: ['GET'])]
    public function applicationDocumentRequests(Application $application, EntityManagerInterface $em, Request $request): JsonResponse
    {
        ...
        $isAuthorized = false;
        if ($employer && $application->getEmployer() === $employer) {
            $isAuthorized = true;
        } elseif ($recruiter && $application->getRecruiter() === $recruiter) {
            $isAuthorized = true;
        }
    ...
    #[Route('/{id}/interview-details', name: 'app_recruiter_application_interview_details', methods: ['GET'])]
    ...
    #[Route('/{id}/todo/create', name: 'app_recruiter_application_createtodo', methods: ['POST'])]
    public function createTodoForApplication(Application $application, Request $request, EntityManagerInterface $em): Response
    {
        ...
        if ($employer && $application->getEmployer() === $employer) {
            ...
            if ($application->getRecruiter() === $recruiter) {
                $isAuthorized = true;
            } elseif ($em->getRepository(JobRecruiter::class)->findOneBy(['job' => $application->getJob(), 'recruiter' => $recruiter])) {
                $isAuthorized = true;
            }
        }
    ...
    #[Route('/{id}/ask-for-document', name: 'app_recruiter_application_askfordocument', methods: ['POST'])]
    public function askForDocument(Application $application, Request $request, EntityManagerInterface $em, EventDispatcherInterface $dispatcher): Response
    {
        ...
        if (!$isAuthorized) {
            ...
            return $this->redirect($referer ?? $this->generateUrl('app_recruiter_applications'));
        }
        ...
        if (empty($documentNames) && empty($singleDocumentName)) {
            ...
            return $this->redirect($referer ?? $this->generateUrl('app_recruiter_job_applications', ['id' => $application->getJob()->getId(), 'applicationId' => $application->getId()]));
        }
        ...
        foreach ($namesToRequest as $name) {
            ...
            if ($recruiter) {
                $documentRequest->setRequestedBy($recruiter);
            }
            ...
            $todo->setProvider($application->getProvider());
            $todo->setEmployer($employer ?? $application->getJob()->getEmployer());
    ...
    #[Route('/{id}/shcudule-interview', name: 'app_recruiter_application_scheduleinterview', methods: ['POST'])]
    ...
    #[Route('/{id}/update-interview/{interviewId}', name: 'app_recruiter_application_updateinterview', methods: ['POST'])]
    ...
    #[Route('/{id}/ask-for-onefile', name: 'app_recruiter_application_askforonefile', methods: ['GET'])]
    ...
    #[Route('/{id}/send-offer', name: 'app_recruiter_application_sendoffer', methods: ['POST'])]
    ...
    #[Route('/{id}/send-contract', name: 'app_recruiter_application_sendcontract', methods: ['GET', 'POST'])]
    ...
    #[Route('/{id}/review-provider', name: 'app_recruiter_application_review_provider', methods: ['GET', 'POST'])]
    public function reviewProvider(
        ...
        if (!$isAuthorized) {
            $this->addFlash('error', "You don't have access to this application.");
            return $this->redirectToRoute('app_recruiter_applications');
        }
        ...
        if ($existingReview) {
            ...
            return $this->redirectToRoute('app_recruiter_job_applications', ['id' => $application->getJob()->getId(), 'applicationId' => $application->getId()]);
        }
    ...
    #[Route('/archive', name: 'app_recruiter_applications_archive', methods: ['POST'])]
    ...
    #[Route('/delete', name: 'app_recruiter_applications_delete', methods: ['POST'])]
    ...
    #[Route('/{id}/delete', name: 'app_recruiter_application_delete', methods: ['GET'])]
    public function delete(Application $application, EntityManagerInterface $em, EventDispatcherInterface $dispatcher): Response
    {
        ...
        return $this->redirectToRoute('app_recruiter_job_applications', ['id' => $application->getJob()->getId()]);
    }

    #[Route('/update-rank', name: 'app_recruiter_application_update_rank', methods: ['POST'])]
    ...
    #[Route('/send-provider-email', name: 'app_recruiter_send_provider_email', methods: ['POST'])]
    public function sendProviderEmail(
        ...
        try {
            ...
            if ($employer && $application->getEmployer() === $employer) {
                ...
                if ($application->getRecruiter() === $recruiter) {
                    $isAuthorized = true;
                } elseif ($entityManager->getRepository(JobRecruiter::class)->findOneBy(['job' => $application->getJob(), 'recruiter' => $recruiter])) {
                    $isAuthorized = true;
                }
            }
    ...
    #[Route('/application/{id}/send-message', name: 'app_recruiter_application_send_message', methods: ['POST'])]
    ...
    #[Route('/application/{id}/messages', name: 'app_recruiter_application_messages', methods: ['GET'])]
    ...
    #[Route('/update-status', name: 'app_recruiter_application_update_status', methods: ['POST'])]
    public function updateStatus(Request $request, EntityManagerInterface $em, EventDispatcherInterface $dispatcher, Registry $workflowRegistry): JsonResponse
    {
        $payload = json_decode($request->getContent(), true);
        $applicationId = $payload['applicationId'] ?? null;
        $transition = $payload['transition'] ?? null;

        if (!$applicationId || !$transition) {
        ...
        if (!$workflow->can($application, $transition)) {
            return $this->json(['success' => false, 'message' => "Invalid transition '$transition' from status '$oldStatus'"], 400);
        }

        try {
            $workflow->apply($application, $transition);
        } catch (\Exception $e) {
        ...
        // Commission Logic for Recruiter
        if ($recruiter && ($newStatus === 'accepted' || $newStatus === 'hired') && $oldStatus !== $newStatus) {
            ...
            if (!$existingInvoice) {
                // Find JobRecruiter record to get commission rate
                $jobRecruiter = $em->getRepository(JobRecruiter::class)->findOneBy([
                    'job' => $application->getJob(),
                    'recruiter' => $recruiter
                ]);

                if ($jobRecruiter) {
                    $commissionRate = $jobRecruiter->getCommissionRate() ?? 0;
                    $job = $application->getJob();

                    // Calculate Amount: Estimated Annual Salary * (Commission / 100)
                    // Use getHourlyRate() * 2080 as a standard estimation for annual salary base
                    $hourlyRate = $job->getHourlyRate();
                    $salaryBase = $hourlyRate ? ($hourlyRate * 2080) : 0;

                    if ($salaryBase > 0 && $commissionRate > 0) {
                        $amount = $salaryBase * ($commissionRate / 100);

                        $invoice = new Invoice();
                        $invoice->setRecruiter($recruiter);
                        $invoice->setEmployer($application->getJob()->getEmployer());
                        $invoice->setJob($application->getJob());
                        $invoice->setProvider($application->getProvider());
                        $invoice->setAmount((int)$amount); // Amount is int? Entity says 'int'. Assuming cents or whole currency?
                        // If entity says int, and it's currency, usually cents.
                        // But let's check Invoice entity again... 
                        // "private ?int $amount = null;"
                        // User prompt didn't specify cents vs dollars. 
                        // let's assume raw value for now or cast to int. 
                        // If salary is 100,000 and rate is 10%, amount is 10,000. Fits in int.

                        $invoice->setParticular("Placement Commission for " . $application->getProvider()->getUser()->getName() . " - " . $job->getTitle());
                        $invoice->setStatus(Invoice::INVOICE_STATUS_PENDING);
                        $invoice->setCreatedAt(new \DateTime());
                        $invoice->setUpdatedAt(new \DateTime());

                        // Generate Invoice Number (Simple unique string)
        ...
        // Dispatch Event
        $dispatcher->dispatch(new ApplicationEvent($application), ApplicationEvent::APPLICATION_STATUS_CHANGED);
